...
const ForgotPasswordModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                addToast("Un lien magique a été envoyé à votre adresse email.", "success");
                onClose();
            } else {
                throw new Error(data.message || "Oups, une erreur est survenue.");
            }
        } catch (error: any) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-rose-900/40 backdrop-blur-md transition-all duration-300">
            <div className="relative w-full max-w-md bg-white dark:bg-gray-900 rounded-[2rem] shadow-2xl p-8 border border-rose-100 dark:border-gray-700 transform scale-100 animate-fadeIn">
                ...
                <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 font-light">Pas de panique, cela arrive même aux meilleures. Entrez votre email pour réinitialiser.</p>
                </div>
...
const SignInForm: React.FC<{ onLoginSuccess: () => void; onForgotPassword: () => void; idPrefix?: string }> = ({ onLoginSuccess, onForgotPassword, idPrefix = 'signin' }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!email.trim() || !password.trim()) {
            addToast("Chérie, n'oubliez pas de remplir tous les champs !", "warning");
            return;
        }
        ...
        try {
            ...
            addToast(error.message || "Identifiants incorrects. Réessayez en beauté !", "error");
        } finally {
            setIsLoading(false);
        }
    };
...
const SignUpForm: React.FC<{ switchToLogin: () => void; idPrefix?: string }> = ({ switchToLogin, idPrefix = 'signup' }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.email || !formData.password || !formData.firstName || !formData.lastName) {
            addToast("Tous les champs sont requis pour rejoindre le club !", "warning");
            return;
        }
        setIsLoading(true);
        try {
            await api.register(formData);
            addToast("Bienvenue ! Votre compte a été créé avec succès.", "success");
            switchToLogin();
        } catch (error: any) {
    ...
    return (
        <form className="w-full space-y-5 animate-fadeIn" onSubmit={handleSubmit} style={{ animationDelay: '0.1s' }}>
            ...
            >
                {isLoading ? 'Création en cours...' : 'Rejoindre la communauté'}
            </button>
    ...
    useEffect(() => {
        ...
        if (hash.includes('?')) {
            ...
            if (error === 'user_exists') {
                addToast("Ce compte existe déjà. Connectez-vous.", "info");
                setIsSignUpActive(false); 
            }
            if (error === 'auth_failed') {
                addToast("L'authentification a échoué.", "error");
            }
